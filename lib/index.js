#!/usr/bin/env node

'use strict'

//
// Use in CLI:
//
//   Type "riot" for help.
//
// Use in Node:
//
//   var riot = require('riot/compiler')
//   riot.make({ from: 'foo', to: 'bar', compact: true })
//   riot.watch({ from: 'foo.tag', to: 'bar.js' })
//

// Include the available actions
const
  Check       = require('./actions/Check'),
  Make        = require('./actions/Make'),
  Watch       = require('./actions/Watch'),
  helpers     = require('./helpers'),
  options     = require('./options'),
  optionator  = require('optionator')(options),
  API         =
  {
    help() { helpers.log(optionator.generateHelp()) },
    version() { return helpers.getVersion() },
    check(opts) { return new Check(opts) },
    make(opts) { return new Make(opts) },
    watch(opts) { return new Watch(opts) }
  }


function cli() {

  // Get CLI arguments

  var args = optionator.parse(process.argv, options)

  console.log(args)

  // Translate args into options hash

  var opts = {
    compiler: {
      compact: args.compact,
      template: args.template,
      type: args.type,
      brackets: args.brackets,
      expr: args.expr,
      modular: args.modular,
      silent: args.silent,
      whitespace: args.whitespace
    },
    ext: args.ext,
    from: args._.shift(),
    to: args._.shift()
  }




  // Call matching method

  var method = Object.keys(API).filter(function(v) { return args[v] })[0]
    || ( opts.from ? 'make' : 'help' )

  API[method](opts)

}


// Run from CLI or as Node module

if (module.parent) {
  module.exports = API
  global.isSilent = true
} else cli()
